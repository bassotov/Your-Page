...
export default function WorkoutGetPage() {
    ...
    if (success === 'true') {
      ...
      if (savedAnswers) {
        try {
          ...
          // If parsing fails, go to landing
          setStep('landing');
        }
      } else {
        setStep('landing');
  ...
  const handlePayment = () => {
    // Save answers before redirecting
    localStorage.setItem('workout-poll-answers', JSON.stringify(answers));

    // Build Polar checkout URL with success redirect
    const successUrl = encodeURIComponent(`${window.location.origin}/workout/get?success=true`);
  ...
  const totalSteps = pollSteps.length + 1; // +1 for details step
  ...
  if (step === 'success') {
    const downloadSkillMd = () => {
      const skill = generateSkillMd(answers);
...
// SKILL.md generator
function generateSkillMd(answers: PollAnswers): string {
  const isRu = answers.language === 'ru';
...
Базовый URL: https://barbash.in/workout
...
### Поля:
- **name**: Название тренировки (с эмодзи)
...
2. Создай URL: \`https://barbash.in/workout?w={base64}\`
...
2. Create URL: \`https://barbash.in/workout?w={base64}\`
